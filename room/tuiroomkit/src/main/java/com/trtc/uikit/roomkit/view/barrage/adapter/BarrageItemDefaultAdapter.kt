...
import com.trtc.uikit.roomkit.view.barrage.EmojiSpan
import com.trtc.uikit.roomkit.view.barrage.viewmodel.DefaultEmojiResource
...
class BarrageItemDefaultAdapter(
    ...
    private var adminIds: Set<String> = emptySet()
    var onRoleInfoChangedListener: (() -> Unit)? = null

    fun setOwnerId(ownerId: String) {
        if (this.ownerId != ownerId) {
            this.ownerId = ownerId
            onRoleInfoChangedListener?.invoke()
        }
    }

    fun setAdminIds(adminIds: Set<String>) {
        if (this.adminIds != adminIds) {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int, barrage: Barrage) {
        ...
        val isOwner = TextUtils.equals(ownerId, barrage.sender.userID)
        val isAdmin = adminIds.contains(barrage.sender.userID)
    ...
    private fun bindRoleBadge(
        ...
        viewHolder.textMsgContent.apply {
            ...
            setTextColor(ResourcesCompat.getColor(context.resources, R.color.roomkit_barrage_g8, context.theme))
            movementMethod = LinkMovementMethod.getInstance()
        }
    }

    private fun bindNormalUser(viewHolder: ViewHolder, barrage: Barrage, fontSize: Int) {
        ...
        viewHolder.textMsgContent.apply {
            ...
            setTextColor(
                ResourcesCompat.getColor(
                    context.resources,
                    R.color.roomkit_barrage_user_name_color,
                    context.theme
                )
            )
        }
    ...
    override fun onCreateViewHolder(parent: ViewGroup): RecyclerView.ViewHolder {
        val view = mLayoutInflater.inflate(R.layout.roomkit_barrage_item_msg, parent, false)
