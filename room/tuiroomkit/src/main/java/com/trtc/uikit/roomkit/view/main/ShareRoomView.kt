...
class ShareRoomView @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null,
    defStyleAttr: Int = 0
) : BaseView(context, attrs, defStyleAttr) {

    private val tvRoomName: TextView by lazy { findViewById(R.id.tv_share_room_name) }
    private val tvRoomID: TextView by lazy { findViewById(R.id.tv_share_room_id) }
    private val btnCopyRoomID: LinearLayout by lazy { findViewById(R.id.btn_share_copy_room_id) }
    private val llPasswordRow: LinearLayout by lazy { findViewById(R.id.ll_share_password_row) }
    private val tvRoomPassword: TextView by lazy { findViewById(R.id.tv_share_room_password) }
    private val btnCopyPassword: LinearLayout by lazy { findViewById(R.id.btn_share_copy_password) }
    private val btnCopyRoomInfo: AppCompatButton by lazy { findViewById(R.id.btn_share_copy_room_info) }

    private var roomStore = RoomStore.shared()
    private var subscribeJob: Job? = null

    init {
        LayoutInflater.from(context).inflate(R.layout.roomkit_view_share_room, this)
        setupListeners()
    }
    ...
    private fun setupListeners() {
        ...
        btnCopyRoomInfo.setOnClickListener {
