...
import com.trtc.uikit.roomkit.base.ui.contactpicker.SelectedContactAdapter
...
class RoomScheduledDetailView @JvmOverloads constructor(
    ...
    private fun isActive(): Boolean = isAttachedToWindow
    ...
    private fun loadRoomInfo() {
        RoomStore.shared().getRoomInfo(roomID, object : GetRoomInfoCompletionHandler {
            override fun onSuccess(roomInfo: RoomInfo) {
                logger.info("getRoomInfo success, roomInfo=$roomInfo")
                this@RoomScheduledDetailView.roomInfo = roomInfo
                if (!isActive()) return
                updateRoomInfoUI()
                updateAttendeesUI()
                fetchAttendees()
            ...
            override fun onFailure(code: Int, desc: String) {
                logger.error("getRoomInfo failed, code=$code, desc=$desc")
                if (!isActive()) return
    ...
    private fun updateRoomInfoUI() {
        ...
        tvStartTime.text = ScheduleDateFormatter.formatDateTime(context, info.scheduledStartTime * 1000L)
        tvDuration.text = formatDurationMinutes(info.scheduledStartTime * 1000L, info.scheduledEndTime * 1000L)
    ...
    private fun showAttendeesDialog() {
        ...
        val view = inflate(context, R.layout.roomkit_dialog_selected_attendees, null)
        ...
        val adapter = SelectedContactAdapter(context)
